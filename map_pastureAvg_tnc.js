...
// Map Title Box
  ...
  info.update = function () {
     this._div.innerHTML = '<div style="background:#fff;opacity:0.7;border-radius:6px;padding:4px 6px"><h4>The Nature Conservancy</h4><span style="font-size:14px"><strong>' + yr + ' ' + mo1 + mo2 + '</strong> &mdash; Average Biomass (lbs/acre)</span></div>';
  };
...
function getPastureStats() {
  ...
  url = 'https://rangesat.nkn.uidaho.edu/api/pasturestats/intra-year/Zumwalt/?year=' + yr + '&start_date=' + mm1 + '-1&end_date=' + mm2 + '-' + dayEnd + '&ranch=TNC';
...
function updateMap(arr) {
  ...
  usersRanch = L.geoJson(ranch, {
    ...
    onEachFeature: function(feature, layer) {
	  ...
	  tbl = tbl + '<tr><td>' + yr + '-' + mo1 + mo2 + '</td><td>TNC</td><td>' + arr[i].pasture + '</td><td>' + (feature.properties.Hectares * 2.471).toFixed(2) + '</td><td>' + (arr[i].biomass_mean_gpm).toFixed(0) + '</td></tr>'; 
	}
      }
  ...
  var printer = L.easyPrint({
     ...
     sizeModes: ['Current'],
     filename: 'pastureAvg_' + yr + '_' + mo1 + mo2 + '_TNC',
     exportOnly: true,
  ...
  map.fitBounds(usersRanch.getBounds());  // zooms to TNC management area
