/* This script creates a Landsat scene THRESHOLD map for authenticated users.
...
let tStep = 50;
let tVals = [1000, 1500];
let tPct1a = 0.4;  // equivalent to 1000 lb/ac for biomass
let tPct1b = 0.4001;
let tPct2a = 0.6;  // equivalent to  1500 lb/ac for biomass
let tPct2b = 0.6001; 
let tLower = 1000;
let tUpper = 1500;
let pop = '</table>';
...
info.update = function () {
   this._div.innerHTML = '<div style="background:#fff;opacity:0.7;border-radius:6px;padding:4px 6px"><h4>' + usrRanch + '</h4><span style="font-size:14px"><strong>' + indicator.toUpperCase() + ', Date: ' + selScene.substring(21,23) + '/' + selScene.substring(23,25) + '/' +  selScene.substring(17,21) + '</strong></span></div>';
   summaryTable();
};
...
$('#indicator').change(function() {
   if(indicator==='biomass'){
     ...
     $('#threshold').attr({ value: '1500', min: '0', max: '2500', step: '1' });
     $('#threshVal').html('1500');
     threshold = 1500;
     ...
     $('#thrMin').html('-1');
     $('#thrMax').html('1');
     $('#threshold').attr({ value: '0.5', min: '-1', max: '1', step: '0.1' });
     $('#threshVal').html('0.5');
     threshold = 0.5;
...
function updateMap() {
   ...
   var printer = L.easyPrint({
      ...
      filename: 'pixelThreshold_' + selDate + '_' + usrRanch,
      exportOnly: true,
